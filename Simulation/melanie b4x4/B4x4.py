...
class Factory(object):
    ...
    def __init__(self, theta_deg_in, lbda_min_nm, lbda_max_nm):
        self.settings = dict(no=1.416, ne=1.424, nSubstrate=1.5, nSuperstrate=1.5, pitch=200, stack=40,
                             angle=theta_deg_in, lbda_min=lbda_min_nm, lbda_max=lbda_max_nm, points=lbda_max_nm - lbda_min_nm + 1, slices=20)
    ...
    def warmup(self):
        ...
        # What's in J:
        #print("Dim 1: " + str(len(J)) + " = nb of wavelengths")
        #print("Dim 2: " + str(len(J[0])) + " = one for r, one for t")
        #print("Dim 3: " + str(len(J[0][0])) + " = one for 1st row, one for 2nd row")
        #print("Dim 4: " + str(len(J[0][0][0])) + " = one for 1st column, one for 2nd column")
        #print("Dim 5 does not exist")
        return J

    def getReflTrans(self):
        J = self.warmup()
        # What's in J: (how to read these results)
        #r_pp = [i[0][0][0] for i in J]  # reflected p component for a p incident wave, one per wl
        #r_ps = [i[0][0][1] for i in J]  # reflected p component for a s incident wave, one per wl
        #r_sp = [i[0][1][0] for i in J]  # reflected s component for a p incident wave, one per wl
        #r_ss = [i[0][1][1] for i in J]  # reflected s component for a s incident wave, one per wl
        #t_pp = [i[1][0][0] for i in J]  # transmitted p component for a p incident wave, one per wl
        #t_ps = [i[1][0][1] for i in J]  # transmitted p component for a s incident wave, one per wl
        #t_sp = [i[1][1][0] for i in J]  # transmitted s component for a p incident wave, one per wl
        #t_ss = [i[1][1][1] for i in J]  # transmitted s component for a s incident wave, one per wl
        result = abs(J) ** 2
        return result
