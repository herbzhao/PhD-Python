...
class preprocessor_class():
    ...
    def splitting_rgb(self, show=True):
        image_b, image_g, image_r = cv2.split(self.image)
        self.image_gray = np.hstack((image_r, image_g, image_b))
        height, width = (np.shape(self.image_gray)[0], np.shape(self.image_gray)[1])
        print(np.shape(self.image))
        self.image_gray = cv2.resize(self.image_gray,(int(width/3), int(height/3)))

        if show is True:
            cv2.imshow("RGB channels", self.image_gray)
            cv2.waitKey(0)
...
if __name__ == "__main__":
    folder_name = r"C:\Users\My Pc\Desktop\20190305 - MCNC 0kjg 1mM"
    image_name = 'MCNC 0kjg 1mM - in dodacane 0.5wt% span80 - 7wt% -20x -001'
    ...
    preprocessor.splitting_rgb()
    # preprocessor.convert_to_grayscale()
    preprocessor.adjust_contrast_and_brightness(alpha=2, beta=-50)
